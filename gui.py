...
def gui(chat):
  ...
  def sendMsg():                  #发送消息
    ...
    chat_ans=chat.main_chat(txtMsg.get('0.0',END))
    strMsg = 'xiao e:' + time.strftime("%Y-%m-%d %H:%M:%S",
                                  time.localtime()) + '\n '
    txtMsgList.insert(END, strMsg, 'greencolor')   #插入到tag位置
    txtMsgList.insert(END, chat_ans+'\n')
    txtMsg.delete('0.0', END)
    gui_log=open("log/gui_log.txt",'a+')
    with open("log/log.txt","r") as f:
      line=f.readlines()
      for l in line:
        gui_log.write(l+'\n')
    gui_log.close()
    f=open("log/gui_log.txt",'r')
    line=f.readlines()
